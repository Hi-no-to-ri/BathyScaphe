...
- (id)fileRefOfResolvedAliasFile
{
    if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber10_9) {
        ...
        NSError *error;
        NSURL *resolvedURL = [NSURL URLByResolvingAliasFileAtURL:url options:0 error:&error];
        if (resolvedURL) {
            return [[self class] fileRefWithFileURL:resolvedURL];
        } else {
            if (error) {
                NSLog(@"SGFileRef: resolving alias error: %@", [error localizedDescription]);
            }
        }
    } else {
...
- (BOOL)isAliasFile
{
    if (floor(NSFoundationVersionNumber > NSFoundationVersionNumber10_9)) {
        NSURL *url = [self fileURL];
        NSNumber *boolNum;
        if (![url getResourceValue:&boolNum forKey:NSURLIsAliasFileKey error:NULL]) {
            return NO;
        }
