...
CFStringRef HTMLFormatter::formatBody()
{
	...
	for (index = 0; index < CFArrayGetCount(datArray()); index++) {
		...
		CFStringRef plainID;
		CFStringRef plainNumber = CFStringCreateWithFormat(kCFAllocatorDefault, NULL, CFSTR("%ld"), index + 1);
		CFStringRef number = CFStringCreateWithFormat(kCFAllocatorDefault, NULL, CFSTR("<a class='number' href='%ld'>%ld</a>"), index+1, index+1);
		CFStringRef idtag;
		...
		if (isAsciiArt(message)) {
			CFStringAppendFormat(body, NULL, CFSTR("<div id='%ld' class='text-art'>"), index + 1);
		} else {
			CFStringAppendFormat(body, NULL, CFSTR("<div id='%ld'>"), index + 1);
		}
