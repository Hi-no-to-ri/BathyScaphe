...
{
    // ステータスコード 500 以降 --> cloudFlare errors
    if (statusCode > 499) {
        if (errorPtr != NULL) {
            NSString *localizedDescription = [NSString stringWithFormat:PluginLocalizedStringFromTable(@"Error 2chReply Status Code", nil, @""), (long)statusCode];
            *errorPtr = [NSError errorWithDomain:SG2chErrorHandlerErrorDomain code:statusCode userInfo:@{NSLocalizedDescriptionKey: localizedDescription}];
        }
        return NO;
    }
    return YES;
