...
- (BOOL)writeForm:(NSDictionary *)forms
{
    ...
    params_ = [self parameterWithForm:forms error:NULL];
...
- (NSString *)parameterWithForm:(NSDictionary *)forms error:(NSError **)errorPtr
{
...
- (BOOL)isResponseValid:(NSInteger)statusCode orError:(NSError **)errorPtr
{
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    if (![self isResponseValid:[(NSHTTPURLResponse *)response statusCode] orError:&error]) {
        [connection cancel];
        if ([self delegate] && [[self delegate] respondsToSelector:@selector(connector:resourceDidFailLoadingWithError:)]) {
            [[self delegate] connector:self resourceDidFailLoadingWithError:error];
        }
    }
}
