...
- (NSData *)postingDataWithID:(NSString *)userID password:(NSString *)password
{
    ...
    NSStringEncoding hoge = CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingEUC_JP);
    ...
    encodedUserID = [userID stringByURLEncodingUsingEncoding:hoge];
    encodedPassword = [password stringByURLEncodingUsingEncoding:hoge];

	forms_ = [NSString stringWithFormat:[[self preferences] be2chAuthenticationFormFormat], encodedUserID, encodedPassword];
	return [forms_ dataUsingEncoding:hoge];
}
...
//	[request_ setValue:[NSBundle applicationUserAgent] forHTTPHeaderField:APP_HTTP_X_2CH_UA_KEY];
    ...
    if (!foo) {
        ...
        if (contents_ && ([contents_ length] > 0)) {
            NSDictionary *userInfo2 = [NSDictionary dictionaryWithObject:([contents_ hasPrefix:@"<html>"] ? @"" : contents_) forKey:NSLocalizedDescriptionKey];
...
- (BOOL)invalidate
{
	...
	if (result_) {
		...
		if (usesKeychain_) {
            NSError *error;
            if (![[self preferences] changeAccount:account_ password:pw_ forType:BSKeychainAccountBe2chAuth error:&error]) {
...
- (NSString *)password
{
	return [[[self class] preferences] passwordForType:BSKeychainAccountBe2chAuth error:NULL];
}
