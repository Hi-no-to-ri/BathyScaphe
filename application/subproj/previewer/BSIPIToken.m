...
#import <SGFoundation/BSURLDownload.h>
...
NSString *const BSIPITokenDidUpdateThumbnailNotification = @"jp.tsawada2.BathyScaphe.ImagePreviewer.BSIPITokenDidUpdateThumbnailNotification";

static NSSize g_requestedThumbnailSize;
...
+ (NSSize)requestedThumbnailSize
{
    return g_requestedThumbnailSize;
}
...
- (BOOL)createThumbnailAndCalcImgSizeForPath:(NSString *)filePath
{
    ...
    if (UTTypeConformsTo(uti, kUTTypePDF)) {
        ...
        [self startCreatingThumbnail:cgImageSource];
...
+ (void)setRequestedThumbnailSize:(NSSize)size
{
    g_requestedThumbnailSize = size;
}

- (NSURL *)convertImgurURLIfNeeded:(NSURL *)originalURL refererNeeded:(BOOL *)boolPtr
{
    NSString *host = [originalURL host];
    if ([host isEqualToString:@"imgur.com"] || [host isEqualToString:@"m.imgur.com"]) {
        NSURL *newURL = [[NSURL alloc] initWithScheme:@"http" host:@"i.imgur.com" path:[originalURL path]];
        *boolPtr = YES;
...
- (id)initWithURL:(NSURL *)anURL destination:(NSString *)aPath
{
	if (self = [super init]) {
        BOOL bar;
        NSURL *adjustedURL = [self convertImgurURLIfNeeded:anURL refererNeeded:&bar];
		ipit_curDownload = [[BSURLDownload alloc] initWithURL:adjustedURL delegate:self destination:aPath referer:(bar ? [anURL absoluteString] : nil) cachePolicy:NSURLRequestReloadIgnoringCacheData];
		if (!ipit_curDownload) {
            return nil;
        }
...
NSImage *thumbnailImageOfImageURL(CGImageSourceRef srcRef, NSInteger size)
{
    NSImage             *thumbnailImg;
    CFDictionaryRef     options;
    CGImageRef          imageRef;

    options = (CFDictionaryRef)([NSDictionary dictionaryWithObjectsAndKeys:
                                 (id)kCFBooleanTrue, (NSString *)kCGImageSourceCreateThumbnailWithTransform,
                                 (id)kCFBooleanTrue, (NSString *)kCGImageSourceCreateThumbnailFromImageIfAbsent,
                                 [NSNumber numberWithInteger:size], (NSString *)kCGImageSourceThumbnailMaxPixelSize,
                                 (id)kCFBooleanTrue, (NSString *)kCGImageSourceCreateThumbnailFromImageAlways,
                                 NULL]);
    
    imageRef = CGImageSourceCreateThumbnailAtIndex(srcRef, 0, options);
    
    thumbnailImg = [[NSImage alloc] initWithCGImage:imageRef size:NSZeroSize];
    
    CGImageRelease(imageRef);
    
    return [thumbnailImg autorelease];
}

- (void)startCreatingThumbnail:(CGImageSourceRef)imageRef
{
    NSImage *qlThumbnail = thumbnailImageOfImageURL(imageRef, [[self class] requestedThumbnailSize].width);
    [self performSelectorOnMainThread:@selector(setThumbnail:) withObject:qlThumbnail waitUntilDone:YES];
}
...
- (void)setThumbnail:(NSImage *)anImage
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:BSIPITokenDidUpdateThumbnailNotification object:self userInfo:nil];
}
...
- (void)bsURLDownloadWillDownloadContentOfUnknownSize:(BSURLDownload *)download
{
	[self setStatusMessage:[self localizedStrForKey:@"Downloading..."]];
}
...
- (BOOL)bsURLDownload:(BSURLDownload *)aDownload shouldDownloadWithDestinationFileName:(NSString *)filename
{
    ...
    // 同名ファイルの存在を確認
    BOOL isDir2;
    if (![[NSFileManager defaultManager] fileExistsAtPath:[aDownload destination] isDirectory:&isDir2] || !isDir2) {
        // 保存先フォルダが削除されている！
        // -postErrorNotification はこの後の -bsURLDownloadDidAbortForDenyingSuggestedFileName: から飛ばされる
        return NO;
    }
