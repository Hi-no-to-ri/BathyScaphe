...
- (void)setInfoFieldTextColorForBackgroundColor:(NSColor *)bgColor
{
    NSColor *tmpColor = [bgColor colorUsingColorSpaceName:NSCalibratedRGBColorSpace];
    CGFloat brightness = [tmpColor brightnessComponent];
    [[self infoField] setTextColor:((brightness < 0.5) ? [NSColor whiteColor] : [NSColor blackColor])];
}
...
- (void)setupTableView
{
	[[self tableView] setDataSource:[BSIPIHistoryManager sharedManager]];
	[[self tableView] setDoubleAction:@selector(changePaneAndShow:)];
	[[self tableView] setVerticalMotionCanBeginDrag:NO];
}
...
//    [[m_infoField cell] setBackgroundStyle:NSBackgroundStyleRaised];
    [self setInfoFieldTextColorForBackgroundColor:color];
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
	if (context == kBSIPIDefaultsContext) {
		if ([keyPath isEqualToString:@"alwaysBecomeKey"]) {
            ...
            [self setInfoFieldTextColorForBackgroundColor:newColor];
			return;
