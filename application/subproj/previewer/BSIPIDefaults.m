...
NSString *const BSIPIDefaultsSaveDirectoryDidChangeNotification = @"jp.tsawada2.BathyScaphe.ImagePreviewer.SaveDirectoryDidChange";
...
- (NSString *)saveDirectory
{
    ...
    if (!saveDirectoryValue) {
        NSURL *downloadsFolderURL = [[[NSFileManager defaultManager] URLsForDirectory:NSDownloadsDirectory inDomains:NSUserDomainMask] objectAtIndex:0];
        return [downloadsFolderURL path];
    }
...
- (void)setSaveDirectory:(NSString *)aString
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:BSIPIDefaultsSaveDirectoryDidChangeNotification object:self];
}
