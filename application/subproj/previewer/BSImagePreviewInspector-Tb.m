...
/*	NSString *path;
    ...
    return [selfBundle imageForResource:name];
}
...
- (NSToolbarItem *)toolbar:(NSToolbar *)toolbar itemForItemIdentifier:(NSString *)itemIdent willBeInsertedIntoToolbar:(BOOL)willBeInserted
{
    ...
    if (![m_toolbarItems objectForKey:itemIdent]) {
        if ([itemIdent isEqual:kIPITbCancelBtnId]) {
            ...
            item = [[[BSSegmentedToolbarItem alloc] initWithItemIdentifier:itemIdent] autorelease];
            ...
            if (floor(NSAppKitVersionNumber) > 1265) { // Yosemite
                [(NSSegmentedControl *)tmp_ setSegmentStyle:8];
            }
