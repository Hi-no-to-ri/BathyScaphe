...
*/
- (void)appendYosemiteStyle:(NSSegmentedControl *)button
{
    if (floor(NSAppKitVersionNumber) > 1265) {
        [button setSegmentStyle:8];
        [button setFrameSize:NSMakeSize(57,25)];
    }
}
...
- (void)initializeToolbarItems:(NSWindow *)aWindow
{
    ...
    item_ = [self appendButton:m_sharingServiceButton
    ...
    [menu setDelegate:(CMRThreadViewer *)wcontroller_];
    [[item_ menuFormRepresentation] setSubmenu:menu];
    [menu release];
	...
	item_ = [self appendToolbarItemWithClass:[BSSegmentedToolbarItem class]
					...
					localizedPaletteLabelKey:st_historySegmentedControlPaletteLabelKey
						 localizedToolTipKey:nil
									  action:NULL
									  target:wcontroller_];
    [self appendYosemiteStyle:m_historyButton];
    [self setupSegmentedControl:m_historyButton onItem:item_ action:@selector(historySegmentedControlPushed:) target:wcontroller_];
	...
	item_ = [self appendToolbarItemWithClass:[BSSegmentedToolbarItem class]
					...
					localizedPaletteLabelKey:st_scaleSegmentedControlPaletteLabelKey
						 localizedToolTipKey:nil
									  action:NULL
									  target:wcontroller_];
    [self appendYosemiteStyle:m_scaleButton];
    [self customizeSegmentedControlIcons:m_scaleButton];
    [self setupSegmentedControl:m_scaleButton onItem:item_ action:@selector(scaleSegmentedControlPushed:) target:wcontroller_];
    
    [topLevelCustomView release];
}
@end
...
{
    if (floor(NSAppKitVersionNumber) > NSAppKitVersionNumber10_7) { // Mountain Lion
