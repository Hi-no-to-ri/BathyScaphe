...
- (void)setupSegmentedControl:(NSSegmentedControl *)viewItem onItem:(NSToolbarItem *)tbItem action:(SEL)action target:(id)target
{
    [viewItem retain];
    ...
    // target と action の設定は viewItem に転送される
    [tbItem setTarget:target];
    [tbItem setAction:action];
}
...
/*            NSString *altImageFilePath = [appSupport pathForImageResource:imageName];
            ...
            NSImage *altImage = [appSupport imageForResource:imageName];
            if (altImage) {
                [control setImage:altImage forSegment:i];
            }
        }
