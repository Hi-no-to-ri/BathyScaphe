...
- (void)initializeToolbarItems:(NSWindow *)aWindow
{
    ...
    [NSBundle loadNibNamed:@"CMRBrowserTbItems" owner:self];
#else
	...
	item_ = [self appendToolbarItemWithClass:[BSSegmentedToolbarItem class]
					...
					localizedPaletteLabelKey:st_viewModeSwitcherItemPaletteLabelKey
						 localizedToolTipKey:st_viewModeSwitcherItemToolTipKey
									  action:NULL
									  target:nil];
	[self setupSwitcherToolbarItem:item_ itemView:[wcontroller_ viewModeSwitcher] delegate:[wcontroller_ document]];
    ...
    [browserTopLevelCustomView release];
}
@end
...
- (void)setupSwitcherToolbarItem:(NSToolbarItem *)anItem itemView:(NSView *)aView delegate:(id)delegate
{
    ...
    [anItem setAction:@selector(toggleThreadsListViewMode:)];
...
- (NSArray *)toolbarDefaultItemIdentifiers:(NSToolbar *)toolbar
{
    if (floor(NSAppKitVersionNumber) > NSAppKitVersionNumber10_7) {
        return [NSArray arrayWithObjects:
                ...
                [self sharingServiceItemIdentifer], // 10.8 and later only.
