...
#import <SGAppKit/BSBoardListCellView.h>
...
- (void)resizeSync:(NSNumber *)numberWithBool
{
    ...
    if ([CMRPref navigationBarShownForClass:[self class]]) {
    ...
    if (!isCollapsed) {
        if (![self threadAttributes]) {
            if (bottomSubview != m_noSelectedThreadsView) {
                [self swapBottomSubview:[self containerView] toView:m_noSelectedThreadsView];
            }
        } else {
...
// 座標が「.5」とかハンパな値になるのを防ぐ（どうも弊害があるように見受けられるので）
...
- (NSImage *)boardListItemIconForItem:(id)boardListItem
{
    if (!boardListItem) {
        return nil;
    }
    ...
    NSString *iconName = [boardListItem iconBaseName];
    ...
    iconName = [iconName stringByAppendingString:@"Template"];
    return [NSImage imageAppNamed:iconName];
}

static NSDictionary *boardListViewBindingOption()
{
    static NSDictionary *cachedDict = nil;
    if (!cachedDict) {
        cachedDict = [@{NSValueTransformerNameBindingOption: NSNegateBooleanTransformerName} retain];
    }
    return cachedDict;
}
...
{
    BSBoardListCellView *result = [outlineView makeViewWithIdentifier:BoardPlistNameKey owner:self];
    UTILAssertKindOfClass(result, NSClassFromString(@"BSBoardListCellView"));
    NSDictionary *dict = boardListViewBindingOption();
    result.textField.stringValue = [item representName];
#if 1
    result.imageView.image = [self boardListItemIconForItem:item];
#else
    result.imageNameBase = [item iconBaseName];
#endif
    if (![result.imageView infoForBinding:@"hidden"]) {
        [result.imageView bind:@"hidden" toObject:CMRPref withKeyPath:@"boardListShowsIcon" options:dict];
        //    } else {
        //        NSLog(@"Already bound!!");
    }
    return result;
...
{
    if ([[aCell objectValue] isKindOfClass:[NSAttributedString class]]) {
        if ([aCell interiorBackgroundStyle] == NSBackgroundStyleDark) {
            if ([[aTableView window] firstResponder] == aTableView) {
                //                NSLog(@"Hyoeeeeee!! backgroundStyle is dark! at row %ld", (long)rowIndex);
                NSAttributedString *attrStr = (NSAttributedString *)[aCell objectValue];
                id attr = [attrStr attribute:NSForegroundColorAttributeName atIndex:0 effectiveRange:NULL];
                if ([[(NSColor *)attr colorUsingColorSpaceName:NSCalibratedWhiteColorSpace] whiteComponent] != 0.0) {
                    NSMutableAttributedString *mAttrStr = [attrStr mutableCopy];
                    [mAttrStr addAttribute:NSForegroundColorAttributeName value:[NSColor whiteColor]];
                    [aCell setObjectValue:mAttrStr];
                    [mAttrStr release];
                    //                } else {
                    //NSLog(@"Fuga");
                }
...
{
    if (aView == [self threadsListTable]) {
        return;
    }
    if ([[self superclass] instancesRespondToSelector:_cmd]) {
        [super view:aView rotateEnough:rotatedDegree];
    }
}
...
- (void)view:(NSView *)aView didFinishRotating:(CGFloat)rotatedDegree
{
    ...
    if ([[self superclass] instancesRespondToSelector:_cmd]) {
        [super view:aView didFinishRotating:rotatedDegree];
    }
}
...
- (void)cleanUpItemsToBeRemoved:(NSArray *)files
{
    ...
    if (![CMRPref sortsImmediately]) {
        if ([self keepPaths] && ([self keepCondition] == CMRAutoscrollWhenThreadDelete)) {
            NSIndexSet *indexes = [[self currentThreadsList] indexesOfFilePathsArray:files ignoreFilter:NO];
            if (indexes) {
                [indexes enumerateIndexesUsingBlock:^(NSUInteger idx, BOOL *stop) {
                    [[self threadsListTable] deselectRow:idx];
                }];
            }
    ...
    // フォーカスの調整
    if ((NSView *)[[self window] firstResponder] == [self textView]) {
        [[self window] makeFirstResponder:[[self threadsListTable] enclosingScrollView]];
    }
...
- (void)reselectThreadIfNeeded
{
    ...
    if ([self keepPaths]) {
        if ([CMRPref sortsImmediately] && ([self keepCondition] == CMRAutoscrollWhenThreadDelete)) {
