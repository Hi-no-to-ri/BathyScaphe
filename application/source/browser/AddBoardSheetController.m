...
- (void)awakeFromNib
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(boardListDidChange:) name:CMRBBSListDidChangeNotification object:[[BoardManager defaultManager] defaultList]];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(defaultListDidSync:) name:BoardWarriorDidFinishSyncingNotification object:[BoardWarrior warrior]];
...
- (IBAction)syncDefaultList:(id)sender
{
    [[BoardWarrior warrior] syncBoardLists];
}

- (IBAction)openAdvancedPref:(id)sender;
{
    [[CMRPref sharedPreferencesPane] showPreferencesPaneWithIdentifier:PPAdvancedPreferencesIdentifier];
}
...
// #warning 64BIT: Check formatting arguments
		...
		if (boardID == NSNotFound) {
            // したらば救済
            NSURL *urlobj = [NSURL URLWithString:url_];
            NSURL *altUrlobj = CMRGetAlternativeHostURLFromShitarabaBoardURL(urlobj);
            if (altUrlobj) {
                boardID = [DBM boardIDForURLString:[[altUrlobj absoluteString] stringByAppendingString:@"/"]];
                if (boardID != NSNotFound) {
                    [DBM renameBoardID:boardID toName:name_];
                    tmpFlag = YES;
                } else {
...
- (void)defaultListDidSync:(NSNotification *)notification
{
    [[self defaultListOLView] reloadData];
}
@end
