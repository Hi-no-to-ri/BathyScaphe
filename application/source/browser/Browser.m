    ...
    if (self = [super init]) {
        ...
        [nc addObserver:self
               selector:@selector(updateThreadsListPartially:)
                 ...
                 object:dbm];
        m_threadListOperationQueue = [[NSOperationQueue alloc] init];
    }
    return self;
}
...
- (void)dealloc
{
    [m_threadListOperationQueue cancelAllOperations];
//    [m_threadListOperationQueue waitUntilAllOperationsAreFinished]; // 危険？？
    [m_threadListOperationQueue release];
...
- (void)setCurrentThreadsList:(BSDBThreadList *)aCurrentThreadsList
{
    ...
    [m_currentThreadsList setOperationQueue:[self threadListOperationQueue]];
    [self didChangeValueForKey:@"threadsListViewMode"];
}

- (NSOperationQueue *)threadListOperationQueue
{
    return m_threadListOperationQueue;
}
...
- (void)setThreadsListViewMode:(NSUInteger)type
{
    ...
    for (id winController in [self windowControllers]) {
...
- (BOOL)segmentedToolbarItem:(BSSegmentedToolbarItem *)item validateSegment:(NSInteger)segment
{
    ...
    return [super segmentedToolbarItem:item validateSegment:segment];
}
...
{
    if ([sender isKindOfClass:[NSSegmentedControl class]]) { return; } // Binding で行われるから
