...
{
    id obj = [[notification userInfo] objectForKey:BSDownloadTaskStatusCodeKey];
    if (obj) {
        NSInteger statusCode = [(NSNumber *)obj integerValue];
        if (statusCode > 499) { // 500以降 cloudFlare error
            m_downloadError = [[NSError errorWithDomain:BSBathyScapheErrorDomain code:BSThreadsListOPTaskCloudFlareStatusError userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:NSLocalizedStringFromTable(@"Invalid Status %ld", @"ThreadsList", @""), (long)statusCode]}] retain];
        }
    }
	m_downloadData = nil;
}
