...
#if defined(DEBUG_THREADTITLE_MODIFICATION)
...
- (void)registerToNotificationCenter
{
	...
	[nc addObserver:self
		   selector:@selector(favoritesManagerDidChange:)
			   name:CMRFavoritesManagerDidLinkOrRemoveFavoritesNotification
			 object:fm];
    [nc addObserver:self
...
- (NSString *)boardName
{
	return [mBoardListItem representName];
}
...
//        NSLog(@"Check\n%@", item);
		...
		NSString *boardID = [NSString stringWithFormat:@"%lu", (unsigned long)[item boardID]];
...
- (id)objectValueForIdentifier:(NSString *)identifier atIndex:(NSInteger)index
{
	...
	ThreadStatus s;
#if defined(DEBUG_THREADTITLE_MODIFICATION)
    static OnigRegexp *hoge = nil;
    OnigResult *match;
    
    if (!hoge) {
        hoge = [[OnigRegexp compile:@"<mark>(.*)</mark>"] retain];
    }
#endif
	...
	if ([identifier isEqualTo:CMRThreadSubjectIndexKey]) {
        ...
        id tmpStr = [row valueForKey:identifier];
        UTILAssertKindOfClass(tmpStr, NSString);
        match = [hoge search:tmpStr];
        if (match) {
            NSString *foo = [match stringAt:1];
            NSRange range = [match rangeAt:0];
            NSString *hogehoge = [tmpStr stringByReplacingCharactersInRange:range withString:foo];
            result = hogehoge;
        } else {
            result = tmpStr;
        }
...
- (void)loadThreadsListAndDownload:(BOOL)forceDL inRebuilding:(BOOL)flag
{
	// 既に起動中の更新タスクを強制終了させる
	[mTaskLock lock];
	if (mTask) {
...
{
    [self loadThreadsListAndDownload:NO inRebuilding:NO];
}

- (void)downloadThreadsList
{
    [self loadThreadsListAndDownload:YES inRebuilding:NO];
}
...
//	}
    ...
    [self loadThreadsListAndDownload:YES inRebuilding:YES];
}


- (void)pushIfCanExcute:(id <CMRTask>)task
{
	if(![task respondsToSelector:@selector(run)]) {
		NSLog(@"%s, %@<%p> dose not respond to -run.",
			  __PRETTY_FUNCTION__,
			  NSStringFromClass([task class]), task);
		return;
	}
    NSOperationQueue *queue = [self operationQueue];
    NSInvocationOperation *op = [[NSInvocationOperation alloc] initWithTarget:task selector:@selector(run) object:nil];
