...
- (void)closeWindowOfAlert:(NSAlert *)alert downloaderFilePath:(NSString *)path
{
    ...
    if (path && [self path] && (![path isEqualToString:[self path]])) {
        return;
    }
    [self cleanUpItemsToBeRemoved:(path ? [NSArray arrayWithObject:path] : nil)];
}
...
{
    if (code == NSOKButton) {
        BoardListItem *item = [controller targetItem];
        if ([BoardListItem isBoardItem:item]) {
            if ([item isEqual:[[self currentThreadsList] boardListItem]]) {
                [[[self currentThreadsList] boardListItem] setRepresentName:[item representName]];
                [self synchronizeWindowTitleWithDocumentName]; // タイトルバーの更新が必要
            }
        }
    }
}
