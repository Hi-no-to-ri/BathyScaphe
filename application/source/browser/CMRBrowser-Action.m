...
static NSInteger expandAndSelectItem(BoardListItem *selected, NSArray *anArray, NSOutlineView *bLT)
{
	...
	for (id eachItem in anArray) {
		// 「閉じているカテゴリ」だけに興味がある
		if (![SmartBoardList isCategory: eachItem] || ![(FolderBoardListItem *)eachItem hasChildren]) {
            continue;
        }
		if (![bLT isItemExpanded:eachItem]) {
            [bLT expandItem:eachItem];
        }
...
// 2010-03-28 tsawada2 修正済
	...
	if ([self shouldLoadThreadAtPath:path_]) {
        id document = [[CMRDocumentController sharedDocumentController] documentAlreadyOpenForURL:[NSURL fileURLWithPath:path_]];
        if (document && (document != self)) {
            // すでに開いている→そのウインドウをアクティブにするだけ
...
- (void)expandSplitView:(BOOL)flag
{
    CGFloat altPosition = [[self splitView] minPossiblePositionOfDividerAtIndex:0];
    CGFloat calcPosition;
    if ([[self splitView] isVertical]) {
        calcPosition = [[self splitView] frame].size.width - [bottomSubview bounds].size.width - [[self splitView] dividerThickness];
    } else {
        calcPosition = [[self splitView] frame].size.height - [bottomSubview bounds].size.height - [[self splitView] dividerThickness];
    }
...
- (IBAction)toggleLayout:(id)sender
{
    ...
    if (idx == 1) {
        if ([CMRPref isSplitViewVertical]) {
            ...
            if (isExpanded) {
                [self expandSplitView:NO];
            }
        }
        if (!isExpanded) {
            [self expandSplitView:YES];
        } else {
