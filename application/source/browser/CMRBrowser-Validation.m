...
- (BOOL)validateDeleteThreadItemsEnabling:(NSArray *)threads
{
	...
	for (NSDictionary *thread_ in threads) {
		NSNumber	*status_;
...
- (BOOL)validateDeleteBoardFromListItem:(NSInteger)tag_
{
	...
	NSInteger numOfSelectedRow = [boardListTable numberOfSelectedRows];
	switch (numOfSelectedRow) {
		case 0:
			return NO;
		case 1:
			return (![SmartBoardList isFavorites:[boardListTable itemAtRow:[boardListTable selectedRow]]]);
		default:
            // 2項目以上選択ならば、少なくともそのうちの1つ以上は「お気に入りではない→削除可能」
			return YES;//(tag_ == kBLDeleteItemViaMenubarItemTag);
	}
}
...
- (BOOL)validateUserInterfaceItem:(id <NSValidatedUserInterfaceItem>)theItem
{
    ...
    if (action_ == @selector(retrieveThread:)) {
        NSView *focusedView_ = (NSView *)[[self window] firstResponder];
        if (focusedView_ == [self textView]) {
            return ([self threadAttributes] != nil);//[super validateUserInterfaceItem:theItem];
        } else {
            // 2010-09-26 不具合あるので使用停止（再取得時 dat 落ちだった場合に何も出ず静かに再取得が止まってしまう）
            return ([[self selectedThreadsReallySelected] count] == 1); // 2013-09-14 不具合治したけど、2個以上だとまだダメ
