...
- (NSUInteger)boardIDForURLString:(NSString *)urlString
{
    ...
    if (!found) {
        const char *boardHost = NULL;
        NSURL *tmp = [NSURL URLWithString:urlString];
        boardHost = [[tmp host] UTF8String];
        
        if (boardHost != NULL && is_2ch_except_pink(boardHost)) {
            NSString *path = [tmp path];
            // http://www.2ch.net とか（この場合 path == @""）、http://search.2ch.net/ のような（この場合 path == @"/"）URL を除外
            // ここでは、http://hoge.2ch.net/fuga/ （この場合 path == @"/fuga"）のパターンを処理しようとしているから。
            if (![path isEmpty] && ![path isEqualToString:@"/"]) {
                query = [NSMutableString stringWithFormat:@"SELECT %@ FROM %@", BoardIDColumn, BoardInfoTableName];
                [query appendFormat:@"\n\tWHERE %@ LIKE '%%%@/'", BoardURLColumn, path];
                cursor = [db performQuery:query];
                if (cursor && [cursor rowCount]) {
                    found = YES;
                }
		 ...
		 toURLString : (NSString *) urlString
{
	...
	if ([db lastErrorID] != 0 && [db lastErrorID] != SQLITE_CONSTRAINT) {
        NSLog(@"Fail insert into %@ (errorCode %ld)", BoardInfoHistoryTableName, (long)[db lastErrorID]);
	...
	[db performQuery : query];
	if ([db lastErrorID] != 0) {
		NSLog(@"Fail update %@ (errorCode %ld)", BoardInfoTableName, (long)[db lastErrorID]);
		[db rollbackTransaction];
...
abort:
	NSLog(@"FAIL append Favorite. Reason : %@", [db lastError]);
	[db rollbackTransaction];
	return NO;
}
...
// 今は未使用
- (NSString *)expressionJoinedByThreadIdentifiers:(NSArray *)identifierStrings
{
    NSMutableString *inExpression = [NSMutableString string];
    
    for (NSString *identifier in identifierStrings) {
        [inExpression appendString:@"'"];
        [inExpression appendString:identifier];
        [inExpression appendString:@"',"];
    }
    if ([inExpression length] > 2) {
        [inExpression deleteCharactersInRange:NSMakeRange([inExpression length]-1, 1)];
    }
    
    return inExpression;
}
// 今は未使用
- (BOOL)removeFavoritesWithThreadIdentifiers:(NSArray *)identifierStrings onBoardID:(NSUInteger)boardID
{
	NSMutableString *query;
	SQLiteDB *db;
	...
	if ([db beginTransaction]) {
		query = [NSMutableString stringWithFormat:@"DELETE FROM %@", FavoritesTableName];
		[query appendFormat:@" WHERE %@ = %lu", BoardIDColumn, (unsigned long)boardID];
		[query appendFormat:@" AND %@ IN (%@)", ThreadIDColumn, [self expressionJoinedByThreadIdentifiers:identifierStrings]];
		[db performQuery:query];
		if ([db lastErrorID] != 0) {
            goto abort;
		}
...
abort:
	NSLog(@"FAIL delete Favorites. Reason: %@", [db lastError]);
	[db rollbackTransaction];
	return NO;
}
...
- (void)setLastWriteDate:(NSDate *)writeDate atBoardID:(NSUInteger)boardID threadIdentifier:(NSString *)identifier
{
	...
	[db performQuery: query];
	if ([db lastErrorID] != 0) {
		NSLog(@"Fail update LastWrittenDate. (errorCode: %ld)", (long)[db lastErrorID]);
	}
}
...
- (BOOL)setThreadStatus:(ThreadStatus)status modifiedDate:(NSDate *)date atBoardID:(NSUInteger)boardID threadIdentifier:(NSString *)identifier
{
	...
	[db performQuery:query];
	if ([db lastErrorID] != 0) {
		NSLog(@"Fail update ThreadStatus. (errorCode: %ld)", (long)[db lastErrorID]);
        return NO;
	}
    return YES;
}
	 ...
	 threadIdentifier:(NSString *)identifier
{
	...
	[db performQuery: query];
	if ([db lastErrorID] != 0) {
		NSLog(@"Fail update IsDatOchi. (errorCode: %ld)", (long)[db lastErrorID]);
	}
}
...
threadIdentifier:(NSString *)identifier
{
	...
	[db performQuery:query];
	if ([db lastErrorID] != 0) {
		NSLog(@"Fail update threadLabel. (errorCode: %ld)", (long)[db lastErrorID]);
	} else {
...
- (BOOL)rebuildFromFilePath:(NSString *)filepath withBoardID:(NSNumber *)boardID isDBError:(BOOL *)dbErrFlagPtr
{
	...
	if ([db lastErrorID] != 0) {
        NSLog(@"Fail to REPLACE at %@. (errorCode = %ld)", NSStringFromSelector(_cmd), (long)[db lastErrorID]);
        if (dbErrFlagPtr != NULL) {
            *dbErrFlagPtr = YES;
        }
		return NO;
						 ...
						 atBoard:(NSUInteger)boardID
{
	...
	if ([self isThreadIdentifierRegistered:identifier onBoardID:boardID numberOfAll:&number]) {
		...
		sql = [NSMutableString stringWithFormat:@"INSERT INTO %@ ( %@, %@, %@, %@, %@, %@, %@, %@ ) VALUES ( %lu, %@, '%@', %lu, %lu, %.0lf, %lu, %lu)",
			   ThreadInfoTableName,
