...
//  Copyright (c) 2004 CocoMonar Project, (c) 2006, 2009, 2014 BathyScaphe Project. All rights reserved.
//  encoding="UTF-8"
//
...
- (void) _showAlertForViaService:(NSInteger)errorCode
{
	NSAlert *alert_;
    NSString *informativeText = [NSString stringWithFormat:NSLocalizedStringFromTable(@"Could Not Open Via Service Message %ld", [self className], nil), (long)errorCode];
		   ...
		   error : (NSString **) error
{
	...
	if ([types containsObject : NSStringPboardType] == NO) {
		*error = @"-[pboard types] dosen't contain NSStringPboardType.";
        ...
        [self _showAlertForViaService:1];
		return;
	}
	...
	if (pboardString == nil) {
		*error = @"pboardString is nil.";
        ...
        [self _showAlertForViaService:2];
		return;
	}
    NSRange testRange = [pboardString rangeOfString:@"://" options:NSLiteralSearch];
    if (testRange.location == NSNotFound) {
        NSString *tmp = [NSString stringWithFormat:@"http://%@", pboardString];
	...
	if (u == nil) {
		*error = @"Can't create NSURL from pboardString.";
        ...
        [self _showAlertForViaService:3];
		return;
	}
