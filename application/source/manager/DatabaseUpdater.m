...
- (BOOL)useProgressPanel;  // default return NO.
 ...
 *  Version 8: ThreadInfo Table の Status で値が18（黄緑色）の項目をすべて2（灰丸）に置き換え済みである事を保証
 *  Version 9: schemaに変更なし。2ch.netに於いてスレッドタイトルの末尾に半角スペースが入っているものから半角スペースを削除
 */
...
@end
@interface DatabaseUpdaterToEight : DatabaseUpdater
@end
@interface DatabaseUpdaterToNine : DatabaseUpdater
@end
...
+ (BOOL)updateFrom:(NSInteger)fromVersion to:(NSInteger)toVersion
{
    ...
    if (fromVersion < 8 && toVersion >= 8) {
        result = [[[[DatabaseUpdaterToEight alloc] init] autorelease] updateDB];
    }
	if(!result) return result;
	
	if (fromVersion < 9 && toVersion >= 9) {
        result = [[[[DatabaseUpdaterToNine alloc] init] autorelease] updateDB];
    }
	if(!result) return result;
...
@implementation DatabaseUpdaterToEight
- (BOOL)useProgressPanel
{
	return YES;
}
...
- (BOOL)updateDB
{
    ...
    if (![self backupDatabase]) {
        return NO;
    }

    [self setInformationText:@"Updating to 8"];
	...
	if ([db beginTransaction]) {
		id query = [NSString stringWithFormat: @"UPDATE %@ SET %@ = '%ld' WHERE %@ = '%ld'",
                    ThreadInfoTableName, ThreadStatusColumn, (long)ThreadLogCachedStatus, ThreadStatusColumn, (long)ThreadHeadModifiedStatus];
		[db cursorForSQL:query];
		if ([db lastErrorID] != 0) goto abort;
		
		if (![self updateVersion:8 usingDB:db]) goto abort;
    ...
    [self setInformationText:@"Finished."];
...
@implementation DatabaseUpdaterToNine
- (BOOL)updateDB
{
	SQLiteDB *db = [[DatabaseManager defaultManager] databaseForCurrentThread];
	if(!db) {
	...
	if([db beginTransaction]) {
		NSString *query = nil;
		id <SQLiteCursor> result = nil;
		id <SQLiteRow> row = nil;
		
		query = @"select boardid, threadid, threadname from BoardThreadInfoView where threadname like '% ' and boardurl like '%.2ch.net%';";
		result = [db performQuery:query];
		if([db lastErrorID] != SQLITE_OK) {
			NSLog(@"Query Error: reaseon %@\n", [db lastError]);
			return NO;
		}
		
		query = @"update threadinfo set threadname=? where boardid=? and threadid=?;";
		SQLiteReservedQuery *reserved = [db reservedQuery:query];
				
		for(NSUInteger i = 0; i < [result rowCount]; i++) {
            @autoreleasepool {
                row = [result rowAtIndex:i];
                id boardid =  [row valueForColumn:@"boardid"];
                id threadid = [row valueForColumn:@"threadid"];
                id threadname = [row valueForColumn:@"threadname"];
                id new = [threadname substringToIndex:[threadname length] - 1];
                threadname = new;
                [reserved cursorForBindValues:[NSArray arrayWithObjects:threadname,boardid,threadid,nil]];
			}
		}
		
		[self updateVersion:9 usingDB:db];
