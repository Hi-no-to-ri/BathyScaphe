...
{
    if (![self threadTitle]) {
        return [super unexpectedStatusErrorMessageText:status];
    }
    return [NSString stringWithFormat:[self localizedString:@"UnexpectedStatusDescription2"], [self threadTitle], (long)status];
}
...
- (CMRDownloaderDataProcessResult)synchronizeLocalDataWithContents:(NSString *)datContents dataLength:(NSUInteger)dataLength error:(NSError **)errorPtr
{
    NSDictionary *thread;
    ...
    if ([[CMRNetGrobalLock sharedInstance] has:[self identifier]]) {
        ...
        return CMRDownloaderDataProcessSuccess; // 失敗扱いにはしない
    }
	...
	if ([CMRPref saveThreadDocAsBinaryPlist]) {
		NSData *data_;
        data_ = [NSPropertyListSerialization dataWithPropertyList:thread format:NSPropertyListBinaryFormat_v1_0 options:0 error:errorPtr];
		if (!data_) {
			result = NO;
		} else {
            result = [data_ writeToURL:[NSURL fileURLWithPath:[self filePathToWrite]] options:NSDataWritingAtomic error:errorPtr];
		}
	} else {
    ...
    return CMRDownloaderDataProcessSuccess;
}
