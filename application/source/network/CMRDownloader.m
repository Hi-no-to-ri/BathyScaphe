...
NSString *const CMRDownloaderContentsNotModifiedNotification = @"jp.tsawada2.BathyScaphe.CMRDownloaderContentsNotModifiedNotification";
...
{
    return [NSString stringWithFormat:[self localizedString:@"UnexpectedStatusDescription"], [[self resourceURLForWebBrowser] absoluteString], (long)status];
}
...
- (void)cancelDownloadWithConnectionFailed:(NSError *)underlyingError
{
	...
	error = [NSError errorWithDomain:BSBathyScapheErrorDomain code:CMRDownloaderConnectionDidFailError userInfo:dict];
	UTILNotifyInfo3(CMRDownloaderConnectionDidFailNotification, error, @"Error");
}

- (void)cancelDownloadWithUnexpectedStatus:(NSInteger)statusCode
{
    NSArray *recoveryOptions;
    NSDictionary *dict;
    NSError *error;
    ...
    messageText = [self unexpectedStatusErrorMessageText:statusCode];
    infoText = [self localizedString:@"UnexpectedStatusInformative"];
...
{
    UTILNotifyName(CMRDownloaderContentsNotModifiedNotification);
}
...
- (void)checkResponse:(NSHTTPURLResponse *)response statusCode:(NSInteger)status forConnection:(NSURLConnection *)connection
{
    switch (status) {
        ...
        case 304:
            ...
            [self cancelDownloadWithNoUpdatedContents];
            
            [self didFinishLoading];
            break;
        case 416:
            [connection cancel];
            // -cancelDownloadWithInvalidPartial より先にやらないと間に合わない！
            if ([[CMRNetGrobalLock sharedInstance] has:[self resourceURL]]) {
        ...
        default:
            [connection cancel];
            [self setMessage:[self localizedCanceledString]];
            [self cancelDownloadWithUnexpectedStatus:status];
...
- (NSURLRequest *)connection:(NSURLConnection *)connection willSendRequest:(NSURLRequest *)request redirectResponse:(NSURLResponse *)redirectResponse
{
	...
	if ([(NSHTTPURLResponse *)redirectResponse statusCode] == 302) {
        ...
        if (tmp && [bbonURLs containsObject:tmp]) {
            ...
            [connection cancel];
            [self setMessage:[self localizedDetectingDatOchiString]];
            [self cancelDownloadWithDetectingDatOchi:tmp];
        }
		[self didFinishLoading];
	}
	return nil;
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    [self checkResponse:http statusCode:status forConnection:connection];
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    CMRDownloaderDataProcessResult result = [self dataProcess:[self resourceData] withConnector:connection];
    if (result == CMRDownloaderDataProcessNoData) {
        UTILNotifyName(CMRDownloaderContentsNotModifiedNotification);
    }
    [self didFinishLoading];
