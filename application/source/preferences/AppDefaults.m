...
static NSString *const AppDefaultsAbondonAllReplyTextViewFeaturesKey = @"Abondon All Reply Text View Features";
static NSString *const AppDefaultsDisableAllReplyTextViewFeaturesKey = @"DisableAllFeaturesOnReplyTextView";
...
static NSString *const AppDefaultsPre243MouseDownTrackingConvertedKey = @"BSMatatabiStepIDPopupSettingConverted";

static NSString *const AppDefaults243IntroShownKey = @"BSIntroShown243";
static NSString *const AppDefaults244IntroShownKey = @"BSIntroShown244";
...
- (void)convertOldMouseDownTrackingTimeSettingMatatabiStep
{
    if (![self isPopupTriggerTypeSet]) {
        double oldTrackingTime = [self mouseDownTrackingTime];
        BSPopupTriggerType newType;
        if (oldTrackingTime < 0.1) {
            newType = BSPopupByMouseClick;
        } else {
            newType = BSPopupByContinuousMouseDown;
        }
        [self setPopupTriggerType:newType];
    }
    
    [[self defaults] setBool:YES forKey:AppDefaultsPre243MouseDownTrackingConvertedKey];
}
...
- (void)convertMisspelledDefaults:(NSString *)misspelledKey to:(NSString *)correctKey
{
    // すでに訂正済み
    if ([[self defaults] objectForKey:correctKey]) {
        return;
    }
    
    id obj = [[self defaults] objectForKey:misspelledKey];
    // 元々、値なし
    if (!obj) {
        return;
    }
    
    [[self defaults] setObject:obj forKey:correctKey];
    [[self defaults] removeObjectForKey:misspelledKey];
    NSLog(@"Misspelled key %@ removed and correct key %@ added.", misspelledKey, correctKey);
}
...
- (BOOL)loadDefaults
{
	NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys: [NSNumber numberWithBool:YES], TS2SoftwareUpdateCheckKey,
                          ...
                          [NSNumber numberWithBool:NO], AppDefaultsPreDotInvaderThemeSettingsConvertedKey,
                          [NSNumber numberWithBool:NO], AppDefaultsPre243MouseDownTrackingConvertedKey,
                          [NSNumber numberWithBool:YES], AppDefaults243IntroShownKey,
                          [NSNumber numberWithBool:NO], AppDefaults244IntroShownKey, NULL];
...
//		[self convertOldFCToThemeFile];
    ...
    if (![[self defaults] boolForKey:AppDefaultsPre243MouseDownTrackingConvertedKey]) {
        [self convertOldMouseDownTrackingTimeSettingMatatabiStep];
    }
    
    [self convertMisspelledDefaults:AppDefaultsAbondonAllReplyTextViewFeaturesKey to:AppDefaultsDisableAllReplyTextViewFeaturesKey];
...
{
    return [[self defaults] boolForKey:AppDefaultsDisableAllReplyTextViewFeaturesKey defaultValue:DEFAULT_ABONDON_ALL_REPLY_TEXT_VIEW_FEATURES];
}
- (void)setAbondonAllReplyTextViewFeatures:(BOOL)flag
{
/*    [[self defaults] setBool:flag forKey:AppDefaultsAbondonAllReplyTextViewFeaturesKey];
    UTILNotifyName(AppDefaultsLayoutSettingsUpdatedNotification);*/
...
#pragma mark SQLite Semi Auto Vacuum
- (NSInteger)previousSQLitePageCount
{
	return [[self defaults] integerForKey:@"SQLitePreviousPageCount"];
}
- (void)setPreviousSQLitePageCount:(NSInteger)pageCount
{
	[[self defaults] setInteger:pageCount forKey:@"SQLitePreviousPageCount"];
}

- (BOOL)isIntroShown
{
    return [[self defaults] boolForKey:AppDefaults244IntroShownKey];
}

- (void)setIntroShown:(BOOL)flag
{
    [[self defaults] setBool:flag forKey:AppDefaults244IntroShownKey];
}
@end
