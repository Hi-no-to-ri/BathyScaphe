...
static NSString *const AppDefaultsUsesRelativeDateFormatKey = @"UsesRelativeDateFormat";
...
//    return [[self threadsListSettingsDictionary] unsignedIntegerForKey:AppDefaultsThreadsListAutoscrollMaskKey defaultValue:DEFAULT_TLSEL_HOLDING_MASK];
    // 以前の設定（今の設定の場合もある）を参照
    CMRAutoscrollCondition oldSetting = [[self threadsListSettingsDictionary] unsignedIntegerForKey:AppDefaultsThreadsListAutoscrollMaskKey defaultValue:DEFAULT_TLSEL_HOLDING_MASK];
    
    // 全くスクロールしないか、ほとんどスクロールするか、の二択
    return (oldSetting == CMRAutoscrollNone) ? CMRAutoscrollNone : DEFAULT_TLSEL_HOLDING_MASK;
}
...
{
    return [[self defaults] boolForKey:AppDefaultsUsesRelativeDateFormatKey defaultValue:DEFAULT_TL_USES_RELATIVE_DATE_FORMAT];
}

- (void)setUsesRelativeDateFormat:(BOOL)flag
{
    [[self defaults] setBool:flag forKey:AppDefaultsUsesRelativeDateFormatKey];
    [[BSDateFormatter sharedDateFormatter] setUsesRelativeDateFormat:flag];
	[self postLayoutSettingsUpdateNotification];
}
