...
#define kPrefNavigationBarShownKeyFormat    @"Navigation Bar Shown (%@)"
...
static NSString *const kPrefPopupTriggerTypeKey = @"Popup Trigger Type";
...
{
    NSString *key = [NSString stringWithFormat:kPrefNavigationBarShownKeyFormat, NSStringFromClass(windowClass)];
    return [[self threadViewerDefaultsDictionary] boolForKey:key defaultValue:DEFAULT_TV_SHOWS_NAVIGATION_BAR];
}

- (void)setNavigationBarShown:(BOOL)flag forClass:(Class)windowClass
{
    NSString *key = [NSString stringWithFormat:kPrefNavigationBarShownKeyFormat, NSStringFromClass(windowClass)];
    [[self threadViewerDefaultsDictionary] setBool:flag forKey:key];
}
...
// See -[AppDefaults convertOldMouseDownTrackingTimeSettingMatatabiStep]
- (BOOL)isPopupTriggerTypeSet
{
    return ([[self threadViewerDefaultsDictionary] objectForKey:kPrefPopupTriggerTypeKey] != nil);
}

- (BSPopupTriggerType)popupTriggerType
{
    return [[self threadViewerDefaultsDictionary] integerForKey:kPrefPopupTriggerTypeKey defaultValue:DEFAULT_TV_POPUP_TRIGGER_TYPE];
}

- (void)setPopupTriggerType:(BSPopupTriggerType)type
{
    [[self threadViewerDefaultsDictionary] setInteger:type forKey:kPrefPopupTriggerTypeKey];
}
