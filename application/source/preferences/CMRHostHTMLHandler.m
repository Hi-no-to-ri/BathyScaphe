...
- (BOOL)parseBoardURL:(NSURL *)boardURL host:(NSString **)hostPtr bbs:(NSString **)bbsPtr
{
    ...
    NSString *path = [boardURL path]; // http://hokkaido.machi.to/hokkaidou-> @"/hokkaidou"
    if (path) {
        NSArray *array = [path componentsSeparatedByString:@"/"]; // @"/hokkaidou" -> {@"/", @"hokkaidou"}
        if ([array count] == 2) {
            if (bbsPtr != NULL) {
...
- (void)addDatLine:(NSArray *)components with:(id)thread count:(NSUInteger *)pLoadedCount
{
    ...
    *pLoadedCount = actualIndex;
    ...
    NSString *hostString = [components objectAtIndex:6];
    NSString *hostFormatString;
    if (![hostString isEmpty]) {
        hostFormatString = [NSString stringWithFormat:@" HOST:%@", hostString];
    } else {
        hostFormatString = @"";
    }

    if (!isThreadTitleParsed) {
        NSString *title_ = [components objectAtIndex:5];
...
- (id)parseHTML:(NSString *)inputSource with:(id)thread count:(NSUInteger)loadedCount lastReadedCount:(NSUInteger *)lastCount
{
    ...
    if (loadedCount == 0 || loadedCount == NSNotFound)  {
        isThreadTitleParsed = NO;
    } else {
        isThreadTitleParsed = YES;
    }
...
/*
ログフォーマットは、
レス番<>投稿者名<>メールアドレス<>日付 時刻 ID<>本文<>サブジェクト（<>ホスト：ver2 のみ）[改行]
