...
- (void)removeFromNotificationCenter
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:BSSSSPIconPlaceholderDidUpdateNotification object:[BSSSSPIconManager defaultManager]];
...
- (id)init
{
	if (self = [super initWithWindow:nil]) {
        m_givenLocationHint = NSZeroPoint;
...
- (void)setContext:(NSAttributedString *)context
{
    ...
    if ([CMRPref showsReferencedMarker]) {
        [[self owner] insertReferencedMarkersForPopupContents:[self textView]];
    }
...
- (NSRect)preparePopUpWindowWithContext:(NSAttributedString *)context owner:(id<CMRThreadViewDelegate, NSTextViewDelegate>)owner locationHint:(NSPoint)point
{
    ...
    m_givenLocationHint = point;
	[self sizeToFit];

	wframe_ = [[self window] frame];
	wframe_.origin = point;
    
    wframe_ = [self constrainWindowFrame:wframe_];
	
    [[self window] setAlphaValue:1.0]; // 0.0
	[[self window] setFrame:wframe_ display:YES];
...
- (void)beIconDidUpdate:(NSNotification *)notification
{
