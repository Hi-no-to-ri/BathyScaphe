...
//  Copyright 2009-2014 BathyScaphe Project. All rights reserved.
//  encoding="UTF-8"
//
...
typedef NS_ENUM(NSUInteger, CMRThreadViewerAlertType) {
...
- (IBAction)deleteThread:(id)sender
{
    ...
    // sender が nil の場合、キーバインド（-[NSResponder doCommandBySelector:]）による呼び出しの可能性がある。
    // このとき、-validateUserInterfaceItem: を経由していないので、ログファイルが存在しないスレッドが対象になって
    // しまっているのに -deleteThread: が呼び出されている可能性がある。
    // ただし、それはスレッド一覧が対象になっているときなので、フォーカスのあるビューがテキストビューでないことをチェックする。
    if (!sender) {
        NSView *focusedView_ = (NSView *)[[self window] firstResponder];
        if (focusedView_ != [self textView]) {
            if (![self validateDeleteThreadItemsEnabling:targets_]) {
                return;
            }
        }
    }
...
- (BOOL)restoreFromRetrieving:(NSString *)path error:(NSError **)error
{
    ...
    if ([signature isEqual:[self threadIdentifier]]) {
        ...
        [self setNotShownThreadRetrieving:NO];
    }
    return YES;
}
...
//            [alert runModal];
    ...
    if ([aSignature isEqual:[self threadIdentifier]]) {
        ...
        [self setNotShownThreadRetrieving:YES];
