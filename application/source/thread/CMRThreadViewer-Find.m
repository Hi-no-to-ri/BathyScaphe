					 ...
					 range: (NSRange) aRange
{
	...
	if (useRegExp && NO == [self validateAsRegularExpression: [searchOptions searchString]]) goto ErrNotFound;

	[self findText: [searchOptions searchString]
...
- (void)findTextByFilter:(NSString *)aString searchMask:(CMRSearchMask)searchOption targetKeys:(NSArray *)keysArray locationHint:(NSPoint)location
{
	...
	UTILNotifyName(BSThreadViewerWillStartFindingNotification);
//    [[self findBarViewController] setStatusText:@"Extracting..." status:YES];
...
//        foo = @"Not found";
        ...
        goto CleanUp;
//    } else {
//        foo = [NSString stringWithFormat:@"%lu found", (unsigned long)nCount];
    }
...
//    [[self findBarViewController] setStatusText:foo status:NO];
}
...
- (IBAction) findAllByFilter : (id) sender
{
	...
	findOperation_ = [[TextFinder standardTextFinder] currentOperation];
//    findOperation_ = [[self findBarViewController] representedObject];
	if (nil == findOperation_)
		return;
	
	[self findTextByFilter: [findOperation_ searchString]
				searchMask: [findOperation_ optionMasks]
				targetKeys: [findOperation_ targetKeysArray]
//                targetKeys:[BSSearchOptions keysArrayFromTargetsMask:[findOperation_ targetsMask]]
...
- (IBAction) findAll : (id) sender
{
	...
	if (useRegExp && NO == [self validateAsRegularExpression: [findOperation_ searchString]]) return;
	...
	found = [self hiliteForMatchingString: [findOperation_ searchString]
...
- (void)extractMessagesWithIDString:(NSString *)IDString fromIDField:(BOOL)isOnIDField forObject:(id)object popUpLocation:(NSPoint)location
{
	CMRThreadLayout *layout = [self threadLayout];
    ...
    for (CMRThreadMessage *message in [layout allMessages]) {
...
{
    [self extractMessagesWithIDString:IDString fromIDField:YES forObject:nil popUpLocation:location];
}
@end
