...
//@class BSFindBarViewController;

@interface CMRThreadViewer : CMRStatusLineWindowController<NSTextViewDelegate, CMRThreadViewDelegate, BSMessageSampleRegistrantDelegate, NSMenuDelegate, BSThreadFileLoadingOperationDelegate>
{	
	// History
...
//    BSFindBarViewController     *m_findBarViewController;
	...
	struct {
        ...
        unsigned int browser_retrieving:1; // ブラウザウインドウで、スレッド一覧の選択スレッドを再取得作業中→「2ペイン」または「3ペインだが、表示中のスレッドとは異なるスレッド」
		unsigned int reserved:28;
	} _flags;
}

@property(readwrite, assign) BOOL shouldResetScaling;
...
- (void)extractMessagesWithIDString:(NSString *)IDString fromIDField:(BOOL)isOnIDField forObject:(id)object popUpLocation:(NSPoint)location; // Available in Matatabi-Step and later.
@end
