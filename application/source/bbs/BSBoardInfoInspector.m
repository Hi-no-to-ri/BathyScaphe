...
- (void)dealloc
{
    ...
    [m_editor release];
    [m_editBoardSheetController setDelegate:nil];
...
- (void)updateHostSymbolsMatrix
{
    ...
    NSArray *allSymbols = [BrdMgr availableHostSymbolsForBoard:[self currentTargetBoardName]];

    NSInteger firstInteger = [m_firstHostSymbolButton tag];
    for (NSInteger i = 0; i < 19; i++) {
        NSInteger tag = firstInteger + i;
        NSButton *button = [[[m_panes tabViewItemAtIndex:2] view] viewWithTag:tag];
        if (i < [allSymbols count]) {
            [button setTitle:allSymbols[i]];
            [button setHidden:NO];
...
- (void)setCurrentTargetBoardName:(NSString *)newTarget
{
    ...
    [self willChangeValueForKey:@"shouldAlwaysMaruLogin"];
    [self willChangeValueForKey:@"shouldAlwaysP22chLogin"];
    ...
    [self willChangeValueForKey:@"shouldEnableMaruButton"];
    [self willChangeValueForKey:@"shouldEnableP22chButton"];
    ...
    [self didChangeValueForKey:@"shouldEnableP22chButton"];
    [self didChangeValueForKey:@"shouldEnableMaruButton"];
    ...
    [self didChangeValueForKey:@"shouldAlwaysP22chLogin"];
    [self didChangeValueForKey:@"shouldAlwaysMaruLogin"];
...
{
    [[self editBoardSheetController] setTargetItem:[self boardListItem]];
...
- (IBAction)hostSymbolChanged:(id)sender
{
    ...
    NSInteger baseTag = [m_firstHostSymbolButton tag];
    for (NSInteger i = 0; i < 19; i++) {
        NSButton *button = [[[m_panes tabViewItemAtIndex:2] view] viewWithTag:(baseTag + i)];
        if ([button isHidden]) {
            continue;
        }
...
{
    return (BSLoginPolicyDecidedByUser == [BrdMgr typeOfBeLoginPolicyForBoard:[self currentTargetBoardName]]);
}

- (BOOL)shouldEnableMaruButton
{
    return (BSLoginPolicyDecidedByUser == [BrdMgr typeOfMaruLoginPolicyForBoard:[self currentTargetBoardName]]);
}

- (BOOL)shouldEnableP22chButton
{
    return (BSLoginPolicyDecidedByUser == [BrdMgr typeOfP22chLoginPolicyForBoard:[self currentTargetBoardName]]);
}
...
{
    return [BrdMgr alwaysMaruLoginAtBoard:[self currentTargetBoardName]];
}

- (void)setShouldAlwaysMaruLogin:(BOOL)checkboxState
{
    [BrdMgr setAlwaysMaruLogin:checkboxState atBoard:[self currentTargetBoardName]];
}

- (BOOL)shouldAlwaysP22chLogin
{
    return [BrdMgr alwaysP22chLoginAtBoard:[self currentTargetBoardName]];
}

- (void)setShouldAlwaysP22chLogin:(BOOL)checkboxState
{
    [BrdMgr setAlwaysP22chLogin:checkboxState atBoard:[self currentTargetBoardName]];
}
