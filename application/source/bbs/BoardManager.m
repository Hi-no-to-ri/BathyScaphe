...
- (NSDictionary *)samba24Info
{
    if (!m_samba24Info) {
        NSURL *url;
        BOOL isDir;
        NSString *userFilePath = [[CMRFileManager defaultManager] supportFilepathWithName:BSSamba24TxtFile resolvingFileRef:NULL];
        if ([[NSFileManager defaultManager] fileExistsAtPath:userFilePath isDirectory:&isDir] && !isDir) {
            url = [NSURL fileURLWithPath:userFilePath];
        } else {
            url = [[NSBundle mainBundle] URLForResource:@"samba24" withExtension:@"txt"];
        }
        m_samba24Info = [[self parseSamba24txt:url] retain];
    }
    return m_samba24Info;
}
...
- (NSString *)boardNameForURL:(NSURL *)theURL
{
    ...
    if (boardID == NSNotFound) {
        NSURL *altURL;
        // したらば救済
        // 旧ホスト←→新ホスト相互変換した URL でもう一度探してみる
        altURL = CMRGetAlternativeHostURLFromShitarabaBoardURL(theURL);
        
        if (altURL) {
            boardID = [dbm boardIDForURLString:[[altURL absoluteString] stringByAppendingString:@"/"]];
        }
    }
