...
- (void)detectorDidFinish:(NSNotification *)aNotification
{
    ...
    [self setBBSSlipValue:[infoDict_ objectForKey:kBSSTDBBSSlipTypeKey] forBoard:board];
...
- (BOOL)editBoardItem:(id)item newBoardName:(NSString *)newBoardName
{
    if (!newBoardName || !item) {
        NSBeep();
        return NO;
    }
    
    NSString *tmp = [[[item representName] copy] autorelease];
    
    [[self defaultList] rename:newBoardName toItem:item];
    [[self userList] rename:newBoardName toItem:item];
    
     if ([[[BSBoardInfoInspector sharedInstance] currentTargetBoardName] isEqualToString:tmp]) {
        [[BSBoardInfoInspector sharedInstance] setCurrentTargetBoardName:newBoardName];
        [[[BSBoardInfoInspector sharedInstance] window] update];
    }
...
{       
    ...
    for (id eachItem in boardItemsForRemoval) {
...
@implementation BoardManager(Samba)
- (NSDictionary *)parseSamba24txt:(NSURL *)fileURL
{
    NSString *fileContents = [[NSString alloc] initWithContentsOfURL:fileURL encoding:NSShiftJISStringEncoding error:NULL];
    ...
    [fileContents enumerateLinesUsingBlock:^(NSString *line, BOOL *stop) {
        ...
        if ([array count] > 1) {
            [mutableDict setObject:[array objectAtIndex:1] forKey:[array objectAtIndex:0]];
        }
    }];
...
- (NSTimeInterval)samba24ValueForHost:(NSString *)serverHost bbs:(NSString *)bbs
{
    if (bbs) {
        id sambaValue = [[self samba24Info] objectForKey:bbs];
        if (sambaValue) {
            return [(NSString *)sambaValue doubleValue];
        }
    }

    if (serverHost) {
        id sambaServerValue = [[self samba24Info] objectForKey:serverHost];
        if (sambaServerValue) {
            return [(NSString *)sambaServerValue doubleValue];
        }
    }
