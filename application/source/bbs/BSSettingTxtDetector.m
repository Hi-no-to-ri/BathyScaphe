...
NSString *const kBSSTDBBSSlipTypeKey = @"BBS_SLIP";
...
- (void)detectBoardPropertiesFromSettingTxtFile:(NSString *)filePath
{
    ...
    BSLoginPolicyType typeValue = BSLoginPolicyDecidedByUser;
    BOOL nanashiOK = YES;
    ...
    while (eachItem = [iter_ nextObject]) {
        ...
        if ([ary2 containsObject:@"BBS_NONAME_NAME"]) {
            ...
            if (![[ary2 objectAtIndex:1] isEqualToString:@""]) {
                typeValue = BSLoginPolicyMandatory;
            }
        ...
        } else if ([ary2 containsObject:@"BBS_SLIP"]) {
            slipValue = [ary2 objectAtIndex:1];
        }
    }
    ...
    returnDict = [NSDictionary dictionaryWithObjectsAndKeys:
                  ...
                  slipValue, kBSSTDBBSSlipTypeKey,
