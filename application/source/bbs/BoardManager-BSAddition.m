...
// Available in BathyScaphe 2.4 "Kazusa-Ushiku" and later.
static NSString *const NNDAlwaysMaruLoginKey = @"AlwaysMaruLogin";
static NSString *const NNDAlwaysP22chLoginKey = @"AlwaysP22chLogin";
...
- (BOOL) alwaysBeLoginAtBoard:(NSString *) boardName
{
	...
	if ((policy_ == BSLoginPolicyUnavailable) || (policy_ == BSLoginPolicyNoAccount)) {
		return NO;
	
	} else if (policy_ == BSLoginPolicyMandatory) {
		return YES;
...
- (BOOL)alwaysMaruLoginAtBoard:(NSString *)boardName
{
	BSLoginPolicyType policy;
	
	policy = [self typeOfMaruLoginPolicyForBoard:boardName];
	
	if ((policy == BSLoginPolicyUnavailable) || (policy == BSLoginPolicyNoAccount)) {
		return NO;
	} else if (policy == BSLoginPolicyMandatory) {
		return YES;
	} else {
        id entry_ = [self entryForBoardName:boardName];
        
        if ([entry_ isKindOfClass:[NSDictionary class]] && [[entry_ allKeys] containsObject:NNDAlwaysMaruLoginKey]) {
            return [entry_ boolForKey:NNDAlwaysMaruLoginKey];
        }
    }
    return [CMRPref shouldLoginIfNeeded];
}

- (void)setAlwaysMaruLogin:(BOOL)flag atBoard:(NSString *)boardName
{
    [self setBoolValue:flag forKey:NNDAlwaysMaruLoginKey atBoard:boardName];
}

- (BOOL)alwaysP22chLoginAtBoard:(NSString *)boardName
{
    BSLoginPolicyType policy;
	
	policy = [self typeOfP22chLoginPolicyForBoard:boardName];
	...
	if ((policy == BSLoginPolicyUnavailable) || (policy == BSLoginPolicyNoAccount)) {
        ...
        if ([entry_ isKindOfClass:[NSDictionary class]] && [[entry_ allKeys] containsObject:NNDAlwaysP22chLoginKey]) {
            return [entry_ boolForKey:NNDAlwaysP22chLoginKey];
        }
    }
    return [CMRPref usesP22chForReply];
}

- (void)setAlwaysP22chLogin:(BOOL)flag atBoard:(NSString *)boardName
{
    [self setBoolValue:flag forKey:NNDAlwaysP22chLoginKey atBoard:boardName];
}
...
- (BSLoginPolicyType)typeOfBeLoginPolicyForBoard:(NSString *)boardName
{
	if (![CMRPref availableBe2chAccount]) {
		return BSLoginPolicyNoAccount;
    }
	...
	if (NULL == hs) {
		return BSLoginPolicyDecidedByUser;
    }

    // Be は BBSPINK でも使えるようになった。2014-05-29
//	if (!is_2ch_except_pink(hs)) {
    if (!is_2channel(hs)) {
        return BSLoginPolicyUnavailable;
    }
...
- (BSLoginPolicyType)typeOfMaruLoginPolicyForBoard:(NSString *)boardName
{
    if (![CMRPref x2chUserAccount]) {
        return BSLoginPolicyNoAccount;
    }
    ...
    if ([[[self URLForBoardName:boardName] path] isEqualToString:@"/maru"]) {
        return BSLoginPolicyMandatory;
    }
    
	return BSLoginPolicyDecidedByUser;
}

- (BSLoginPolicyType)typeOfP22chLoginPolicyForBoard:(NSString *)boardName
{
    if (![CMRPref p22chUserAccount]) {
        return BSLoginPolicyNoAccount;
    }
...
- (void) setTypeOfBeLoginPolicy: (BSLoginPolicyType) aType forBoard: (NSString *) boardName
{
	if (aType == BSLoginPolicyDecidedByUser) return; // Currently not need to record it
...
- (BOOL)needToDetectNoNameForBoard:(NSString *)boardName shouldInputManually:(BOOL *)boolPtr
{
    ...
    if (!date || (fabs([date timeIntervalSinceNow]) > 60*60*24*14)) {
        if (showsDebugLog) {
