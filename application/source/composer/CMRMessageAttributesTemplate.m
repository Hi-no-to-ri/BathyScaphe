...
- (NSAttributedString *)lastUpdatedHeaderAttachment
{
	...
	*/
	...
	if (!st_lastUpdatedHeaderAttachment) {
        ...
        // 1. App Support 内の3分割新着レス画像を検索
        attachment_ = [self lastUpdatedHeaderThreePartAttributedString:kUpdatedHeaderImageName ofCustomized:YES];
        if (!attachment_) {
            // 2. App Support 内の単独新着レス画像を検索
            // 3. 内蔵の新着レス画像を検索
            attachment_ = [self attachmentAttributedStringWithImageFile:kUpdatedHeaderImageName];
            if (!attachment_) {
                // 4. 内蔵の3分割新着レス画像を検索
                attachment_ = [self lastUpdatedHeaderThreePartAttributedString:kUpdatedHeaderImageName ofCustomized:NO];
            }
        }
...
- (NSAttributedString *)hrAttachmentString
{
    NSTextAttachment *attachment = nil;
    BSHorizontalRuleCell *cell;
    NSAttributedString *attrs = nil;
    
    attachment = [[NSTextAttachment alloc] init];
    cell = [[BSHorizontalRuleCell alloc] init];
    if (!cell) {
        [attachment release];
        return nil;
    }
...
- (NSAttributedString *)lastUpdatedHeaderThreePartAttributedString:(NSString *)name ofCustomized:(BOOL)isCustomized
{
    NSTextAttachment *attachment = nil;
    ...
    attachment = [[NSTextAttachment alloc] init];
    cell = [[BSLastUpdatedHeaderCell alloc] initWithImageNameBase:name searchCustomized:isCustomized];
    if (!cell) {
        [attachment release];
        return nil;
    }
...
static NSNumber *underlineStyleWithBoolValue(BOOL hasUnderline)
{
	...
	if (!singleNum || !noneNum) {
		singleNum = [[NSNumber alloc] initWithInteger:(NSUnderlineStyleSingle|NSUnderlineByWordMask)];
...
- (NSMutableDictionary *)messageAttributesForAnchor
{
	if (!_messageAttributesForAnchor) {		
		_messageAttributesForAnchor = [[[self class] defaultAttributes] mutableCopy];
...
- (NSMutableDictionary *)messageAttributesForName
{
	if (!_messageAttributesForName) {
		_messageAttributesForName = [[[self class] defaultAttributes] mutableCopy];
		...
		[self setAttributeInDictionary:_messageAttributesForName
						 attributeName:BSMessageKeyAttributeName
								 value:BSMessageKeyAttributeNameField];
	}
...
- (NSMutableDictionary *)messageAttributesForTitle
{
	if (!_messageAttributesForTitle) {
		_messageAttributesForTitle = [[[self class] defaultAttributes] mutableCopy];
...
- (NSMutableDictionary *)messageAttributes
{
	if (!_messageAttributes) {
		...
		_messageAttributes = [[[self class] defaultAttributes] mutableCopy];
...
- (NSMutableDictionary *)messageAttributesForText
{
	if (!_messageAttributesForText) {
		_messageAttributesForText = [[[self class] defaultAttributes] mutableCopy];
...
- (NSMutableDictionary *)messageAttributesForBeProfileLink
{
	if (!_messageAttributesForBeProfileLink) {
		_messageAttributesForBeProfileLink = [[[self class] defaultAttributes] mutableCopy];
...
- (NSMutableDictionary *)messageAttributesForHost
{
	if (!_messageAttributesForHost) {
		_messageAttributesForHost = [[[self class] defaultAttributes] mutableCopy];
