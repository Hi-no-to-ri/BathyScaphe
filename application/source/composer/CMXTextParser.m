...
*/
+ (NSString *)cachedMessageWithMessageSource:(NSString *)aSource swapSsspScheme:(BOOL)flag
{
    NSMutableString *tmp;
    NSString *result;
    
    tmp = [aSource mutableCopy];
    [self convertMessageSourceToCachedMessage:tmp swapSsspScheme:flag];
...
{
    return [self cachedMessageWithMessageSource:aSource swapSsspScheme:NO];
}
...
static void htmlConvertDeleteAllTagElements(NSMutableString *theString)
{
    ...
    while ((result_ = [theString rangeOfString:@"<"
        ...
        // 削除しない要素
        {
            ...
            // <ul>
            if (0 == strcasecmp(tagName, "ul")) {
                shouldDelete = NO;
//            } else if (0 == strcasecmp(tagName, "font")) {
//                shouldDelete = NO;
            } else if (0 == strcasecmp(tagName, "hr")) {
                shouldDelete = NO;
            }
        }
...
+ (void)convertMessageSourceToCachedMessage:(NSMutableString *)aSource swapSsspScheme:(BOOL)flag
{
    @synchronized([CMXTextParser class]) {
        static NSArray *removal = nil;

        static NSString *ssspFrom = nil;
        static NSString *ssspTo = nil;
        ...
        if (!ssspFrom) {
            ssspFrom = [[BSSsspLinkScheme stringByAppendingString:@"://"] retain];
            ssspTo = [[BSInnerSsspLinkScheme stringByAppendingString:@"://"] retain];
        }
        
        htmlConvertBreakLineTag(aSource);
        [aSource replaceCharacters:[NSString backslash] toString:[NSString yenmark]];
        // Unicode 方向制御文字を削除
        for (NSString *hoge in removal) {
            [aSource deleteCharacters:hoge];
        }
        htmlConvertDeleteAllTagElements(aSource);
        if (flag) {
            [aSource replaceCharacters:ssspFrom toString:ssspTo];
            // ちょっとトリッキーかなぁ
            [aSource replaceCharacters:@"<hr>" toString:[NSString stringWithCharacter:NSAttachmentCharacter]];
        }
...
{
    [self convertMessageSourceToCachedMessage:aSource swapSsspScheme:NO];
}
...
static NSDate *dateWith2chDateString(NSString *theString)
{
    ...
    // それでもダメだった場合 -- open2ch
    CFDateFormatterSetFormat(kDateFormatterAlt, CFSTR("yy/MM/ddHH:mm"));
    date_ = (NSDate *)CFDateFormatterCreateDateFromString(NULL, kDateFormatterAlt, (CFStringRef)dateString_, NULL);
    if (date_) {
        return [date_ autorelease];
    }
    
    if ([dateString_ length] > 9 && [[dateString_ substringWithRange:NSMakeRange(5, 5)] isEqualToString:@"03/32"]) {
        [dateString_ replaceCharactersInRange:NSMakeRange(5, 5) withString:@"04/01"];
        date_ = (NSDate *)CFDateFormatterCreateDateFromString(NULL, kDateFormatterStd, (CFStringRef)dateString_, NULL);
        if (date_) {
            return [date_ autorelease];
        }
...
static BOOL _parseExtraField(NSString *extraField, CMRThreadMessage *aMessage)
{
    ...
    */
    ...
    static NSCharacterSet *allHostSymbolCharacterSet = nil;
    ...
    /*
     2005-02-03 tsawada2<tsawada2@users.sourceforge.jp>
     extraField が 0 または O 一文字の場合は、携帯・PCの区別記号と見なして直接処理
     ログファイルのフォーマットの互換性などを考慮して、Host の値として処理することにする。
     2014-11-24 最新の実装は以下に基づく：
     OSX専用2chブラウザ -BathyScaphe- 潜航深度 53m
     http://anago.2ch.net/test/read.cgi/mac/1409140597/699
     > ちなみに現在設定している文字種は、
     >
     > 0123456789@ABCDEGHKMOQSWXabdehoprx*+-!
    */
    if (!allHostSymbolCharacterSet) {
        NSString *tmp = SGTemplateResource(@"System - AllHostSymbols");
        allHostSymbolCharacterSet = [[NSCharacterSet characterSetWithCharactersInString:tmp] retain];
    }

    if (length_ == 1) {
        unichar c = [extraField characterAtIndex:0];
        if ([allHostSymbolCharacterSet characterIsMember:c]) {
            [aMessage setHost:extraField];
            return YES;
        }
    } else if (length_ == 2) {
        unichar c = [extraField characterAtIndex:0];
        NSString *last = [extraField substringWithRange:NSMakeRange(1, 1)];
        if ([allHostSymbolCharacterSet characterIsMember:c] && [last isEqualToString:@"!"]) {
            [aMessage setHost:extraField];
            return YES;
        }
    }
    ...
    NSRange matchedBERange = NSMakeRange(0, 0);
    if (matchOfBE) {
        matchedBERange = [matchOfBE bodyRange];
        NSString *beStr_ = [matchOfBE stringAt:1];
        [aMessage setBeProfile:[beStr_ componentsSeparatedByString:@"-"]];
        if (matchedBERange.location == 0) { // extraField の先頭でマッチ＝ID, Host が無い（ID, Host が有れば BE より前に書かれている）
            return YES;
        }
    }

    // HOST
    if (matchOfBE) { // HOST と BE が両方ある場合。BE でマッチして処理済みの文字列はカットする
        matchOfHOST = [regExpForHOST search:[extraField substringWithRange:NSMakeRange(0, matchedBERange.location)]];
    } else {
...
static BOOL _parseDateExtraField(NSString *dateExtra, CMRThreadMessage *aMessage)
{
    ...
    NSRange range = [dateExtra rangeOfString:@"NY:AN:NY.AN" options:NSLiteralSearch];
    if (range.location != NSNotFound) {
        // 差し替え
        dateExtra = [dateExtra stringByReplaceCharacters:@"NY:AN:NY.AN" toString:@"00:00:00.00"];
        nyanNyan = YES;
    }
    ...
    if (nyanNyan) {
        // 画面表示上は NY:AN:NY.AN に戻すべき
        NSString *tmp = [aMessage dateRepresentation];
        [aMessage setDateRepresentation:[tmp stringByReplaceCharacters:@"00:00:00.00" toString:@"NY:AN:NY.AN"]];
    }
